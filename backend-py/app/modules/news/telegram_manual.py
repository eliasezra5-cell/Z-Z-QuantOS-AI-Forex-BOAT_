"""Telegram manual-forward news ingestion (additive).

Mirrors ``whatsapp_manual.py``: any inbound Telegram message that is not a
recognized bot command is treated as a news event. It is saved into the same
``NewsItem`` store used by the realtime collectors, pushed through the unified
AI pipeline (``news:ingest`` -> analysis -> ``news:processed`` -> 5-agent
...
def ingest_telegram_manual(chat_id, text):
    """Save a manual-forward Telegram message as a news event.

    Returns the persisted item. When called from an async context the
    coroutine is scheduled on the running loop; otherwise it runs
    ...
    item = {
        "sourceId": None,
        "source": f"Telegram {chat_id}" if chat_id else "Telegram",
        ...
        "channel": str(chat_id or ""),
        "time": now_ms,
        "raw": {"sourceType": SOURCE_TYPE, "chatId": str(chat_id or "")},
    }
    try:
